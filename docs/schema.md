...
| 場所            | 列                         | 備考                                   |
| --------------- | -------------------------- | -------------------------------------- |
| `products`      | `lead_time_days` (null 可) | NULL なら店舗の既定値                  |
| `shop_settings` | `default_lead_time_days`   | 既定 7 日                              |
| `shop_settings` | `default_cover_days`       | 入荷後に持っておきたい日数。既定 14 日 |
| `shop_settings` | `large_order_threshold`    | 通知する注文金額。既定 30,000 円       |

商品ごとの設定を NULL 可にしているのは、全商品に個別設定を強いると運用されなくなるため。既定値で回り、輸入品など特別なものだけ上書きする。

### stock_velocity(p_window_days)

発注推奨のもとになる販売実績を返す。**予測の計算はここでは行わない**（`packages/core/src/reorder.ts` が担当）。DB も React も起動せずに予測式を検証できる状態を保つため。

**販売数を注文（`orders.ordered_at`）から数える。** 在庫台帳の時刻は在庫が動いた瞬間であって売れた瞬間ではない。取り込んだ過去の注文は在庫移動が取り込み時に記録されるので、台帳基準では販売ペースが跳ね上がる。

`observed_from`（最初の在庫移動と最初の注文の早い方）を返すのは、新しい商品を期間全体で割って販売ペースを過小評価しないため。

### notifications

送信済み通知の記録。`dedupe_key` に一意制約。

| dedupe_key の形          | 単位          |
| ------------------------ | ------------- |
| `low_stock:2026-09-21`   | 1日1回        |
| `large_order:<order_id>` | 注文ごとに1回 |

書き込みは `claim_notification()` 経由のみで、この関数は **service_role にだけ** EXECUTE を許可している（通知を送るのは定期処理なので、画面からは呼ばせない）。読み取りは認証済み全員に開いており、設定画面で「通知が飛んだか」を確認できる。

### claim_notification(kind, dedupe_key, payload)

通知を「これから送る」と宣言する。`insert ... on conflict do nothing` の影響行数で判定し、新しく記録できたときだけ true を返す。

確認と記録を別のクエリに分けないのは、その隙間で複数の Worker が同時に「まだ送っていない」と判断しうるため。一意制約に判定させることで、この隙間が構造上なくなる。
